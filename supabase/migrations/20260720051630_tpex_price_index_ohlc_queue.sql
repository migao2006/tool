...
add constraint historical_archive_scope_check_tpex check (
    ...
    and (
        ...
        or (
            ...
            and schema_version = 'tpex_price_index_ohlc.v1'
        )
    )
    ...
    and (
        ...
        or (
            provider_code = 'TPEX'
            and source_dataset = 'tpex_price_index_ohlc'
            and scheduled_market = 'TPEX'
            and asset_type = 'BENCHMARK'
            and source_symbol = 'TPEX_INDEX'
        )
        or (
            provider_code = 'FINMIND'
...
alter table market_data.historical_archive_objects
validate constraint historical_archive_scope_check_tpex;

alter table market_data.historical_archive_objects
drop constraint historical_archive_scope_check;

alter table market_data.historical_archive_objects
rename constraint historical_archive_scope_check_tpex
to historical_archive_scope_check;

alter table market_data.historical_backfill_tasks
add constraint historical_backfill_task_identity_check_tpex check (
        ...
        or (
            ...
            and (
                ...
                or (
                    ...
                    and market = 'TPEX'
                    and provider_code = 'TPEX'
                    and source_dataset = 'tpex_price_index_ohlc'
                    and selection_basis = 'FIXED_TPEX_MONTH_REQUEST'
                    and requested_start_date
                    = date_trunc('month', requested_start_date)::date
                    and requested_end_date = (
...
alter table market_data.historical_backfill_tasks
validate constraint historical_backfill_task_identity_check_tpex;

alter table market_data.historical_backfill_tasks
drop constraint historical_backfill_task_identity_check;

alter table market_data.historical_backfill_tasks
rename constraint historical_backfill_task_identity_check_tpex
to historical_backfill_task_identity_check;

alter table market_data.historical_backfill_tasks
add constraint historical_backfill_task_research_scope_check_tpex check (
...
alter table market_data.historical_backfill_tasks
validate constraint historical_backfill_task_research_scope_check_tpex;

alter table market_data.historical_backfill_tasks
drop constraint historical_backfill_task_research_scope_check;

alter table market_data.historical_backfill_tasks
rename constraint historical_backfill_task_research_scope_check_tpex
to historical_backfill_task_research_scope_check;

-- noqa: disable=PG01
create index if not exists historical_backfill_tasks_tpex_month_claim_idx
...
create or replace function market_data.seed_tpex_price_index_ohlc_tasks(
    p_start_month date,
    p_end_month date,
...
begin
    ...
    if p_end_month >= current_taipei_month then
        raise exception using
            errcode = '22023',
            message = 'TPEX OHLC queue accepts completed calendar months only';
    end if;
    ...
    with months as materialized (
        ...
        left join lateral (
            ...
            where archive.provider_code = 'TPEX'
              and archive.source_dataset = 'tpex_price_index_ohlc'
              and archive.source_symbol = 'TPEX_INDEX'
              and archive.scheduled_market = 'TPEX'
...
begin
    ...
    set
        ...
        updated_at = now()
    where expired.provider_code = 'TPEX'
      and expired.source_dataset = 'tpex_price_index_ohlc'
      and expired.source_symbol = 'TPEX_INDEX'
      and expired.market = 'TPEX'
    ...
    with candidate as materialized (
        ...
        where queued.provider_code = 'TPEX'
          and queued.source_dataset = 'tpex_price_index_ohlc'
          and queued.source_symbol = 'TPEX_INDEX'
          and queued.market = 'TPEX'
          and queued.asset_type = 'BENCHMARK'
          and queued.selection_basis = 'FIXED_TPEX_MONTH_REQUEST'
...
begin
    if not exists (
        ...
        where task.task_id = p_task_id
          and task.provider_code = 'TPEX'
          and task.source_dataset = 'tpex_price_index_ohlc'
          and task.source_symbol = 'TPEX_INDEX'
          and task.market = 'TPEX'
          and task.asset_type = 'BENCHMARK'
          and task.selection_basis = 'FIXED_TPEX_MONTH_REQUEST'
    ) then
        return false;
    end if;
...
create or replace function market_data.tpex_price_index_ohlc_backfill_snapshot(
    p_start_month date,
    p_end_month date
)
returns table (
...
comment on function market_data.seed_tpex_price_index_ohlc_tasks(
    date,
    date,
    timestamptz
) is
'Seeds completed Gregorian months for official TPEx price-index OHLC.';

comment on function market_data.claim_tpex_price_index_ohlc_task(
    text,
    uuid,
    integer
) is 'Claims one isolated official TPEx monthly OHLC task.';

comment on function market_data.complete_tpex_price_index_ohlc_task(
    bigint,
    uuid,
    boolean,
    date,
    ...
    integer,
    text
) is 'Completes only an isolated official TPEx monthly OHLC task.';

revoke all on function market_data.seed_tpex_price_index_ohlc_tasks(
    date,
    date,
    timestamptz
) from public, anon, authenticated;

revoke all on function market_data.claim_tpex_price_index_ohlc_task(
    text,
    uuid,
    integer
) from public, anon, authenticated;

revoke all on function market_data.complete_tpex_price_index_ohlc_task(
    bigint,
    uuid,
    boolean,
    date,
...
revoke all on function market_data.tpex_price_index_ohlc_backfill_snapshot(
    date,
    date
) from public, anon, authenticated;

grant execute on function market_data.seed_tpex_price_index_ohlc_tasks(
    date,
    date,
    timestamptz
) to service_role;

grant execute on function market_data.claim_tpex_price_index_ohlc_task(
    text,
    uuid,
    integer
) to service_role;

grant execute on function market_data.complete_tpex_price_index_ohlc_task(
    bigint,
    uuid,
    boolean,
    date,
...
grant execute on function market_data.tpex_price_index_ohlc_backfill_snapshot(
    date,
    date
) to service_role;
