...
台股智選是「可驗證的台股機會排序系統」，不是 AI 猜漲跌。v20.2 以不可修改的 point-in-time 推薦快照為公開讀取來源，分開計算短波段（2／3／5／10 個交易日）與中期（前台綜合 10／20／40 個交易日，約 2～8 週）候選，並以交易成本、下跌風險與換手懲罰調整為成本後機會排名。60 日只保留為研究訊號，不進入公開中期排行榜。

首頁採最近一次成功快照先顯示與背景局部更新；五頁為首頁、短波段、中期機會、自選股、策略驗證中心。沒有獨立預測頁、投資紀錄、持股成本或損益管理；既有 `portfolio_positions` 表與資料不刪除，但前台不讀寫。大型語言模型只負責整理與解釋，不決定排名；未完成 Walk-forward 校準或有效樣本少於 100 筆時，不顯示勝率或預期報酬。
