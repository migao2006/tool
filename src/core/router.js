import { hasImplicitAuthCallback } from "../features/auth/auth-callback.js?v=auth-2";
...
export function createRouter({ canActivate = () => true } = {}) {
  ...
  function show(route, { updateHash = true, restoreScroll = false, stockKey = null } = {}) {
    ...
    const targetHash = targetStockKey
      ...
      : `#${targetRoute}`;
    const authCallbackPending = hasImplicitAuthCallback();
    if (!authCallbackPending && updateHash && window.location.hash !== targetHash) {
  ...
  function start() {
    ...
    show(
      hasImplicitAuthCallback() ? "home" : window.location.hash.slice(1),
      { updateHash: false },
    );
  }
