...
def test_manifest_records_p2_freshness_refactoring_and_auth_boundaries() -> None:
    ...
    snapshot = hardening["prediction_snapshot"]
    ...
    refactoring = hardening["p2_refactoring"]
    assert refactoring["public_venue_adapters_preserved"] is True
    assert refactoring["remote_status"] == "REPOSITORY_ONLY_NOT_REMOTELY_VERIFIED"

    auth = hardening["auth_recovery"]
    assert auth["flow_type"] == "pkce"
    assert auth["recovery_event"] == "PASSWORD_RECOVERY"
    assert auth["redirect_allowlist_status"] == "NOT_REVERIFIED_BY_THIS_PATCH"
