"""Auditable contracts for the TWSE raw price/volume research feature set."""
...
class FeatureValueAudit:
    ...
    def __post_init__(self) -> None:
        if self.feature_name not in TWSE_PRICE_VOLUME_FEATURE_NAMES:
            raise ValueError("unsupported TWSE price/volume feature")
...
@dataclass(frozen=True)
class TwsePriceVolumeFeatureRow:
    """One decision-time row that cannot hide missing or future inputs."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.horizon != 5:
            raise ValueError("TWSE research features support only horizon=5")
        if self.feature_schema_version != TWSE_PRICE_VOLUME_FEATURE_SCHEMA_VERSION:
            raise ValueError("unexpected feature schema version")
        if self.feature_schema_hash != TWSE_PRICE_VOLUME_FEATURE_SCHEMA_HASH:
            raise ValueError("unexpected feature schema hash")
        if self.price_basis != TWSE_PRICE_VOLUME_PRICE_BASIS:
        ...
        if self.usage_scope != "FEATURE_RESEARCH_ONLY":
            raise ValueError("TWSE feature rows must remain research-only")
        if self.system_status != "RESEARCH_ONLY":
            raise ValueError("TWSE feature rows cannot be promoted by this builder")


@dataclass(frozen=True)
class TwsePriceVolumeFeatureBuildResult:
    input_row_count: int
    trading_sessions: tuple[date, ...]
    rows: tuple[TwsePriceVolumeFeatureRow, ...]
