...
- 代理可自行讀寫專案、安裝或更新必要依賴、執行命令與測試、建立或套用 migration、匯入資料、管理 RLS／Auth／Edge Functions、設定必要環境變數、檢查 logs、建立提交與 PR、推送、合併、部署、重新部署及修復失敗流程。
...
- 所有程式與設定修改必須使用 Git 留下紀錄並推送至 GitHub；不得只修改遠端而不保留可追溯來源。
