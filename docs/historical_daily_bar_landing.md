...
> 2026-07-19 核對：Production R2 已封存 TWSE 1,080 檔與 TPEX 607 檔；TPEX 尚有 284 檔待回補，ETF 尚未開始。全部仍為 raw research archive，詳見 [`current-status.md`](current-status.md)。
...
```powershell
uv run python -m scripts.probe_finmind_historical `
  --symbols "2330,2317" `
...
```powershell
uv run python -m scripts.import_historical_daily_bars `
  --symbols "2330,2317" `
  --start-date 2021-07-19 `
  --end-date 2026-07-17 `
...
模型資料。每次 workflow 預設最多處理 300 檔（每組 credential 最多 100 檔）；工作開始時先讀取 FinMind 額度，保留安全額度後依
...
目前 campaign 的固定結束日是 2026-07-17；它完成後不會自動把 R2 日線延伸到新交易日。每日近期資料另由 current import 保存於 Supabase；長期 R2 累積仍需後續 daily delta archive workflow。
