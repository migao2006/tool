...
- 部署與遠端資料庫變更仍須遵守第六章及第九章；已授權的 GitHub push／合併可由既有整合自動觸發 Vercel 部署，不必另行確認，但未經當次明確授權不得直接操作 Vercel。
...
- 已獲授權的 GitHub push 或 PR 合併，允許透過既有 GitHub／Vercel 整合自動觸發 Preview 或 Production 部署，不必再次詢問或事前提醒。
- 自動觸發部署不等於授權直接操作部署平台。未經使用者在當次任務明確授權，禁止使用 Vercel CLI、API 或 MCP 建立、提升、回復、刪除部署，或修改專案、網域及環境變數。
- 自動部署失敗時可先做唯讀診斷並回報；重新部署、回復版本或變更遠端設定前仍須取得明確授權。
