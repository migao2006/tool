...
> 2026-07-19 核對：下表表示 provider client 已實作，不代表憑證目前有效、資料已排程匯入、歷史 vintage 完整或可用於正式 point-in-time 模型。實際資料狀態見 [`current-status.md`](current-status.md)。

本專案只保存真實回應及來源追蹤資訊。API 缺漏、憑證未設定或回應格式錯誤時，必須回報
`RESEARCH_ONLY`／`FAIL`，不得產生假行情或假預測。

## 已實作的 provider client
...
| FinMind | `src/data/providers/finmind.py` | 多年行情、籌碼、財報、公司行動與下市資料 | `FINMIND_TOKEN`、`FINMIND_TOKEN_SECONDARY`、`FINMIND_TOKEN_TERTIARY` |
...
第二、第三組 FinMind token 只供歷史回補 credential worker 隔離使用；一般單次 FinMind client
仍只讀取自己的單一 token，不得把三組 token 合併、輪替或輸出。

目前 API readiness workflow 只驗證一般 provider 設定與 primary FinMind 小型請求，不代表 secondary／tertiary FinMind 或 R2 read/write 已通過。歷史回補必須以各 worker 結果、Supabase manifest 與 R2 audit 為準。

## 安全設定

`.env.example` 只列一般本機欄位；secondary／tertiary FinMind 是 GitHub-worker-only secrets。實際正式金鑰應放在 GitHub Repository Secrets：
...
多年歷史行情排程另使用 GitHub Repository Variables `R2_ACCOUNT_ID` 與 `R2_BUCKET_NAME`。
三個 FinMind token 只能分別注入自己的 worker；R2 credential 只授權 private archive bucket 的
Object Read & Write。瀏覽器與 Vercel 前端不得取得上述任何 secret。
...
```powershell
uv run python -m scripts.check_data_apis
```

執行小型真實請求；缺少憑證的來源會顯示 `NOT_CONFIGURED`，不會使用假資料：

```powershell
uv run python -m scripts.check_data_apis --live
```
...
```powershell
uv run python -m scripts.fetch_api_data TWSE `
  --dataset daily_bars `
