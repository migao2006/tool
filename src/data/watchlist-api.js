...
import { PredictionApiError, requestPredictionApi } from "./api-client.js?v=api-3";
...
export async function setWatchlistMembership({
  ...
  return requestPredictionApi(`watchlist/${encodeURIComponent(normalizedSymbol)}`, {
    method: selected ? "PUT" : "DELETE",
    body: selected ? { symbol: normalizedSymbol } : undefined,
    accessToken: token,
    signal,
    config,
  });
