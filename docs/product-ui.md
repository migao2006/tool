...
上市／上櫃使用資料集切換，不是同一份清單的 `ALL` 篩選器。兩個市場必須各自保存 loading、empty、error 與快照狀態，排名只在該市場內顯示；上櫃尚無結果時顯示真實空狀態，不得 fallback 上市資料。資料集內可再篩選產業、決策、資料品質、流動性、Rank Score、校準後上漲機率與 cost profile。ETF 暫不出現在普通股票候選頁。前端不得把 Rank Score、機率、分位數、波動或市場狀態重新加權。
