-- `current_time` is also a PostgreSQL built-in returning time with time zone.
-- On PostgREST RPC execution it could resolve to the built-in instead of the
-- PL/pgSQL variable, causing timestamptz > timetz and stopping every deep job.
