...
const MODEL_VERSION='v16.1-market-aware-history-cash-ttm';
...
async function snapshotHistory(symbol){
  const payload=await fetchJson('/data/latest.json?v=16.1',30000,0),groups=Object.values(payload?.groups||{}).flat(),match=groups.find(row=>row?.stock?.symbol===symbol),history=match?.analysis?.priceHistory;
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js?v=16.1',{updateViaCache:'none'}).catch(()=>{});
