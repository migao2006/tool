...
const S={
  ...
  dataHealth:null,dataHealthState:'loading'
};
...
async function loadStocks(){
  ...
  try{
    ...
    render();loadFundamentals();loadDataHealth();
  }catch(error){
...
async function loadDataHealth(force=false){
  S.dataHealthState='loading';
  try{
    S.dataHealth=await fetchJson(`${EDGE}?type=data-health${force?'&refresh=1':''}`,45000,0);
    S.dataHealthState='ready';
  }catch{
    S.dataHealthState='error';
  }
  render();
}
...
function healthPercent(row){
  const direct=safe(firstValue(row,['coveragePercent','coverage','percent','progress','completion','ratio']));
  if(direct!=null)return clamp(direct<=1?direct*100:direct,0,100);
  const covered=safe(firstValue(row,['covered','coveredCount','ready','completed','verified','official'])),total=safe(firstValue(row,['total','totalCount','expected','target','eligible']));
  return covered!=null&&total?clamp(covered/total*100,0,100):null
}
function healthStatusInfo(){
  const raw=typeof S.dataHealth?.overallStatus==='object'?S.dataHealth.overallStatus?.status:S.dataHealth?.overallStatus;
  const status=String(raw||'').toLowerCase();
  if(['healthy','ok','ready','complete','green'].includes(status))return{className:'ok',label:'資料來源正常'};
  if(['error','failed','critical','red'].includes(status))return{className:'bad',label:'部分來源異常'};
  if(S.dataHealthState==='loading')return{className:'',label:'健康檢查中'};
  if(S.dataHealthState==='error')return{className:'bad',label:'健康資訊暫缺'};
  return{className:'',label:'部分資料累積中'}
}
function statusCard(){
  const rev=S.stocks.filter(x=>instrumentGroup(x)!=='etf'&&(x.revenue!=null||x.rev!=null)).length,fin=S.stocks.filter(x=>x.roe!=null||x.eps!=null).length,chip=S.stocks.filter(x=>x.foreign!=null||x.inst!=null).length;
  const health=healthStatusInfo(),sourceCount=healthRows(S.dataHealth?.sources).length;
  const detail=sourceCount?`${sourceCount} 個來源已檢查 · 點此查看涵蓋率與缺漏原因`:`月營收 ${rev} 檔 · 財報 ${fin} 檔 · 法人 ${chip} 檔${S.fundPeriod?` · ${S.fundPeriod}`:''}`;
  return`<button type="button" class="card data-health data-health-button" data-health-open aria-haspopup="dialog"><span><b>資料健康中心</b><span class="muted">${detail}</span></span><span class="status-pill ${health.className}">${health.label}<span aria-hidden="true">›</span></span></button>`
}
function healthSourceHtml(row,index){
  const label=firstValue(row,['label','name','source','key'])||`資料來源 ${index+1}`;
  const status=String(firstValue(row,['status','state'])||'').toLowerCase();
  const statusLabel=['healthy','ok','ready','complete'].includes(status)?'正常':['error','failed','critical'].includes(status)?'異常':['missing'].includes(status)?'缺漏':'部分';
  const statusClass=statusLabel==='正常'?'':statusLabel==='異常'?'bad':'warn';
  const date=firstValue(row,['latestDate','latest','dataDate','date','period','updatedAt','lastSuccessAt'])||'尚未回傳';
  const percent=healthPercent(row),covered=firstValue(row,['covered','coveredCount','ready','completed']),total=firstValue(row,['total','totalCount','expected','target']);
  const reason=firstValue(row,['missingReason','reason','error','message','note'])||'沒有回報缺漏原因';
  const missing=firstValue(row,['missing','missingCount']);
  return`<div class="health-source"><div class="head"><div><b>${esc(label)}</b><div class="muted">資料日期 ${esc(date)}</div></div><span class="tag ${statusClass}">${statusLabel}</span></div>${percent==null?(covered==null?'':`<div class="health-progress-label"><span>已涵蓋</span><b>${fmt(covered,0)} 筆${missing!=null?` · 缺 ${fmt(missing,0)}`:''}</b></div>`):`<div class="health-progress-label"><span>涵蓋率${covered!=null&&total!=null?` ${fmt(covered,0)}／${fmt(total,0)}`:''}</span><b>${fmt(percent,0)}%</b></div><div class="progress" role="progressbar" aria-label="${esc(label)}涵蓋率" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${fmt(percent,0)}"><span style="width:${percent}%"></span></div>`}<div class="muted small">${esc(reason)}</div></div>`
}
function healthGroupHtml(row,index){
  const key=String(firstValue(row,['key','group','market','id'])||'').toLowerCase(),labels={listed:'上市',otc:'上櫃',etf:'ETF'};
  const label=firstValue(row,['label','name'])||labels[key]||`分組 ${index+1}`,percent=healthPercent(row);
  const ready=firstValue(row,['ready','completed','covered','coveredCount','verified','official']),total=firstValue(row,['total','target','expected','totalCount','eligible']);
  return`<div class="health-group"><div class="health-progress-label"><b>${esc(label)}</b><span>${ready!=null&&total!=null?`${fmt(ready,0)}／${fmt(total,0)}`:percent==null?'等待後端統計':`${fmt(percent,0)}%`}</span></div><div class="progress" role="progressbar" aria-label="${esc(label)}累積進度" aria-valuemin="0" aria-valuemax="100"${percent==null?'':` aria-valuenow="${fmt(percent,0)}"`}><span style="width:${percent??0}%"></span></div></div>`
}
function dataHealthModalHtml(){
  const sources=healthRows(S.dataHealth?.sources),groups=healthRows(S.dataHealth?.groups),repairs=healthRows(S.dataHealth?.repairQueue),scoreHistory=S.dataHealth?.scoreHistory;
  const missingSummary=healthRows(S.dataHealth?.missingData?.summary),missingExamples=healthRows(S.dataHealth?.missingData?.examples),classificationLabels={scheduled_repair:'可補，已排程修復',upstream_error:'API／來源暫時異常',stale_source:'等待官方更新期別',unavailable_from_source:'公開來源沒有歷史',official_not_provided:'免費官方來源未提供',insufficient_history:'歷史筆數尚未達門檻',not_applicable:'不適用',partial_source:'來源僅提供部分欄位'};
  const datasetLabels={price:'每日行情',price_history:'歷史日線',monthly_revenue:'月營收',quarterly_revenue:'季度營收',quarterly_financials:'季度財報',income:'損益表',balance:'資產負債表',cashflow:'現金流量表',cash_conversion:'盈餘現金轉換',institutional:'法人籌碼',margin:'融資融券',lending:'借券',holdings:'集保持股',benchmark:'市場基準歷史',profile:'ETF 基本資料',etf_profile:'ETF 基本資料',premium_discount:'ETF 折溢價',etf_premium_discount:'ETF 折溢價',tracking_error:'ETF 追蹤誤差',etf_tracking_error:'ETF 追蹤誤差',fees:'ETF 費用',etf_fees:'ETF 費用',concentration:'ETF 集中度',etf_top10_concentration:'ETF 前十大集中度',deep_refresh:'深度資料更新',deep_analysis:'深度分析'};
  const datasetLabel=row=>datasetLabels[firstValue(row,['dataset'])]||firstValue(row,['dataset'])||'資料';
  const historyCount=Array.isArray(scoreHistory)?scoreHistory.length:safe(firstValue(scoreHistory,['count','snapshotCount','days','tradingDays','finalDateCount']));
  const health=healthStatusInfo();
  return`<div class="modal"><div class="sheet health-sheet"><button class="sheet-close" type="button">×</button><div class="head"><div><h2>資料健康中心</h2><div class="muted">公開資料日期、涵蓋率與修復狀態</div></div><span class="status-pill ${health.className}">${health.label}</span></div>
    <section class="health-section"><h3>分組累積進度</h3><div class="health-groups">${groups.length?groups.map(healthGroupHtml).join(''):'<div class="muted">後端正在建立上市、上櫃與 ETF 分組統計。</div>'}</div></section>
    <section class="health-section"><div class="head"><h3>資料來源</h3><button class="btn secondary small-btn" id="refreshDataHealth">重新檢查</button></div><div class="health-sources">${sources.length?sources.map(healthSourceHtml).join(''):'<div class="card warn-card"><b>詳細健康資料暫時無法取得</b><p class="muted">股票頁仍會顯示現有官方資料；稍後可重新檢查。</p></div>'}</div></section>
    <section class="health-section"><h3>缺漏原因判定</h3>${missingSummary.length?`<div class="rules health-classifications">${missingSummary.map(row=>`<span>${esc(datasetLabel(row))} · ${esc(classificationLabels[firstValue(row,['classification'])]||firstValue(row,['classification'])||'待判定')} ${fmt(firstValue(row,['count'])||0,0)} 筆</span>`).join('')}</div>`:'<div class="muted">目前沒有可分類的來源缺漏。</div>'}${missingExamples.length?`<details class="health-issues"><summary>查看個別缺漏範例</summary><div class="health-repairs">${missingExamples.slice(0,20).map(row=>`<div><b>${esc(firstValue(row,['name'])||firstValue(row,['symbol'])||'未命名')} ${esc(firstValue(row,['symbol'])||'')} · ${esc(datasetLabel(row))}</b><span>${esc(firstValue(row,['reason'])||classificationLabels[firstValue(row,['classification'])]||'等待後端判定')}</span></div>`).join('')}</div></details>`:''}${repairs.length?`<div class="muted small" style="margin-top:8px">修復佇列：${repairs.map(row=>`${esc(firstValue(row,['key'])||'項目')} ${fmt(firstValue(row,['value','count','pending','errors'])||0,0)}`).join(' · ')}</div>`:''}</section>
    <section class="health-section"><h3>歷史分數</h3><div class="card ${historyCount?'accent':''}"><b>${historyCount?`已累積 ${fmt(historyCount,0)} 份紀錄`:'歷史分數累積中'}</b><p class="muted">${historyCount?'排名與分數變化會使用已完成的交易日紀錄。':'至少需要兩個不同交易日，才能顯示可信的排名與分數變化。'}</p></div></section>
  </div></div>`
}
function openDataHealth(){modalRoot.innerHTML=dataHealthModalHtml();bindModal()}
...
function peerHtml(stock){
  ...
  const rawRows=peer.metrics||peer.rows||peer.items||[],rows=healthRows(rawRows),basis=String(firstValue(peer,['scope','basis','method','fallbackType'])||'industry').toLowerCase();
...
function bind(){
  ...
  qa('[data-health-open]').forEach(button=>button.onclick=()=>openDataHealth());
...
function bindModal(){
  ...
  q('#refreshDataHealth',modalRoot)?.addEventListener('click',async event=>{event.currentTarget.disabled=true;event.currentTarget.textContent='檢查中…';await loadDataHealth(true);if(modalRoot.innerHTML)openDataHealth()});
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js?v=17.1',{updateViaCache:'none'}).catch(()=>{});
