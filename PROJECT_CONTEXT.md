...
Worker 另外區分「全市場已計分」與「已公開發布」：`scoredCycleKey` 表示完整掃描已完成，`completedCycleKey` 表示該週期已成功建立不可變 run 並切換公開 head。若 Enrichment 在完整掃描期間完成並產生同日 Dirty Queue，Worker 必須保留 `scoredCycleKey`、改走增量重算，待同週期 dirty 工作結清後才發布；不得重新開始全市場掃描而形成發布死鎖。尚未完成的其他 Enrichment 可依既有 `base_ready`／`enriching` 語意留在背景，不得混入已封存 revision。

因此 `targetDataDate` 可以新於 `publishedDataDate`；公開 API 一律跟隨 `v20_publication_head`。當 `dirtyQueuePending` 尚未依發布規則結清時，保留上一個 `publishedDataDate` 是後端不可變發布保護，不是前端快取過期。

不可變發布會一次複製完整市場的多期間訊號；正式環境只對 `twss_v20_publish_recommendation_run(jsonb)` 設定 120 秒 function-scoped statement timeout，以容納有界且原子的發布交易，不修改 Data API `authenticator` 的全域 8 秒限制。
