...
v19 在既有系統上加入預先計算的 AI 排行榜、分頁與篩選、個股分析、官方新聞／公告、深色／淺色主題，以及更清楚的資料日期與背景工作狀態。個人頁僅提供自選清單、候選比較與重要提醒，不提供投資紀錄、持股成本或損益管理。既有 API 與功能保持相容；CORE 與 MARKET 兩個 Supabase 專案維持原用途，沒有搬移、交換或刪除任何既有資料。
