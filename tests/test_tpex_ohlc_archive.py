...
from src.data.ingestion.tpex_ohlc_archive import (
    build_tpex_ohlc_archive,
    build_tpex_ohlc_manifest,
)
from src.data.ingestion.tpex_ohlc_contracts import (
...
OBSERVED_AT = datetime(2026, 7, 20, 5, tzinfo=timezone.utc)
PAYLOAD_HASH = "b" * 64
BUCKET = "alpha-lens-archive"


def _row(index: int, day: int, close: str, change: str) -> TpexOhlcObservation:
    values = (f"2024/01/{day:02d}", "234.13", "234.96", "230.69", close, change)
    return TpexOhlcObservation(
        ...
        open_index=Decimal("234.13"),
        high_index=Decimal("234.96"),
        low_index=Decimal("230.69"),
        close_index=Decimal(close),
    )


def _batch() -> NormalizedTpexOhlcBatch:
    return NormalizedTpexOhlcBatch(
        requested_month=date(2024, 1, 1),
        response_date=date(2024, 1, 1),
        source_version="website.en.indexInfo.inx.v1",
        source_url=(
        ...
        rows=(
            _row(0, 2, "233.23", "-0.78"),
            _row(1, 3, "231.05", "-2.18"),
        ),
    )
...
def test_tpex_uses_provider_scoped_key_and_shared_manifest_contract() -> None:
    batch = _batch()
    artifact = build_tpex_ohlc_archive(batch)
    expected_key = (
        "raw/v1/provider=tpex/dataset=tpex_price_index_ohlc/"
        "scheduled_market=TPEX/asset_type=BENCHMARK/symbol=TPEX_INDEX/"
    ...
    assert artifact.schema_version == "tpex_price_index_ohlc.v1"
    assert artifact.object_metadata()["provider-code"] == "TPEX"
    assert artifact.object_metadata()["source-dataset"] == "tpex_price_index_ohlc"
    ...
    manifest = build_tpex_ohlc_manifest(
        batch,
        artifact,
        bucket_name=BUCKET,
        object_etag='"tpex-etag"',
    )
...
def test_tpex_archive_roundtrips_and_preserves_six_raw_fields() -> None:
    batch = _batch()
    artifact = build_tpex_ohlc_archive(batch)
    ...
    raw_source = json.loads(str(result.rows[0]["source_row"]))
    assert raw_source == [
        "2024/01/02",
        "234.13",
        "234.96",
...
def test_reader_rejects_tpex_ohlc_invariant_violation() -> None:
    batch = _batch()
    artifact = build_tpex_ohlc_archive(batch)
    ...
    table = table.set_column(
        ...
        pa.array([Decimal("230"), Decimal("234.96")], type=pa.decimal128(18, 4)),
    )
    ...
    assert captured.value.reason_code == "TPEX_OHLC_ARCHIVE_INVALID"


@pytest.mark.parametrize(
    ("provider", "dataset", "market"),
    (
        ("TPEX", "daily_bars", "TPEX"),
        ("FINMIND", "tpex_price_index_ohlc", "TPEX"),
        ("TWSE", "tpex_price_index_ohlc", "TPEX"),
        ("TPEX", "taiex_price_index_ohlc", "TWSE"),
    ),
)
