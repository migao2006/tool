"""Serialize official TPEx price-index OHLC into the shared archive contract."""
...
def _pyarrow_modules() -> tuple[Any, Any]:
    try:
        ...
        raise IngestionError(
            ...
            "pyarrow is required to serialize TPEx OHLC history",
        ) from error
    return pa, pq


def tpex_ohlc_schema() -> Any:
    ...
    return pa.schema(
        ...
        metadata={
            b"archive.schema_version": TPEX_OHLC_SCHEMA_VERSION.encode(),
            b"archive.source_dataset": TPEX_MONTHLY_OHLC_DATASET.encode(),
...
def _validate_request(
    ...
    if (
        ...
        raise IngestionError(
            "TPEX_OHLC_ARCHIVE_REQUEST_MISMATCH",
            "TPEx OHLC batch does not match its archive request",
        )


def serialize_tpex_ohlc_parquet(
    batch: NormalizedTpexOhlcBatch,
    *,
    request: HistoricalArchiveRequest,
) -> HistoricalArchiveArtifact:
    ...
    rows = [
        {
            "archive_schema_version": TPEX_OHLC_SCHEMA_VERSION,
    ...
    base_schema = tpex_ohlc_schema()
    schema = base_schema.with_metadata(
        {
            **(base_schema.metadata or {}),
    ...
    return HistoricalArchiveArtifact(
        ...
        row_count=len(rows),
        schema_version=HISTORICAL_ARCHIVE_SCHEMA_VERSIONS[TPEX_MONTHLY_OHLC_DATASET],
