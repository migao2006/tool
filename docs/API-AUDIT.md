...
v16.3-ui5 另提供單檔歷史日線按需補齊。明細 API 先讀 `stock_price_history`；不足 120 筆且未有「來源本來較短」完成標記時，Edge Function 先取得單檔租約、再次查庫，再原子保留中央帳本的 1 次 FinMind 額度、發出一次 `TaiwanStockPrice` 請求、合併較新的官方快照並持久化。互動補抓每小時另限 30／60 次並保留 10／20 次排程額度；仍共用 300／600 次總上限。額度不足時回傳 HTTP 202 `HISTORY_PENDING` 與重試時間，不再將尚未累積誤報為 HTTP 502；供應商實際錯誤則另標 `HISTORY_UPSTREAM_ERROR`。
...
- 個股有當日行情但尚未輪到深度累積時，屬 `stock_price_history` 暫缺；v16.3-ui5 會按需補抓並保存。中央額度已滿是 `HISTORY_PENDING`，不是 API 故障，也不應顯示為 502。
...
| `stock_analysis_cache` | 最新分析 | 僅研究結果欄位可公開只讀；原始錯誤、重試與修復欄位限管理端 |
| `opportunity_score_history` | 點時分數 | 只讀 |
| `stock_sync_state` | 分組游標、處理數、錯誤與成功時間 | 僅 `service_role`／管理端 |
| `app_admins` | 已授權的網站管理員帳號與啟用狀態 | RLS；一般登入者僅能確認自己的列，不能寫入 |

上述公開市場資料表皆啟用 RLS，訪客與一般登入者沒有新增、修改或刪除權限。`data_sync_status` 是未使用的舊表名，fresh migration 不再依賴它；正式同步進度只由 `twss_admin_operations_log()` 在再次驗證 `auth.uid()` 與 `app_admins.active` 後回傳，公開快照不輸出。排程經 Vault 權杖呼叫 Edge Function，函式再以伺服器角色寫入，並將健康／缺漏摘要寫入 `[admin-data-health]` 管理日誌。原始碼只包含 publishable key，不包含 service role key 或同步權杖。
