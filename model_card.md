...
<!-- release-manifest:model-header:start -->
> 最後核對：2026-07-24。OOS 驗證 workflow：[`29690820942`](https://github.com/migao2006/tool/actions/runs/29690820942)；最新具完整 artifact／provenance 證據的橫截面研究推論 workflow：[`29701335309`](https://github.com/migao2006/tool/actions/runs/29701335309)；最新特徵 workflow：[`29693937930`](https://github.com/migao2006/tool/actions/runs/29693937930)；發布 commit：未記錄於目前可用證據（不得推測）。動態資料與阻塞現況見 [`docs/current-status.md`](docs/current-status.md)，部署證據邊界見 [`docs/release-state.md`](docs/release-state.md)。

> 本區塊與下方具完整 artifact／provenance 證據的快照由 `release-manifest.json` 產生；請勿直接修改。
<!-- release-manifest:model-header:end -->
...
- 政策評估狀態：`MISSING_REQUIRED_DATA=672`
- Hard fail：0
...
Production Supabase 與前端可以讀取這批歷史 OOS 研究結果；legacy publisher 曾將
全部列記為 `NO_TRADE`，新契約則因 formal policy evidence 不完整而 fail closed 為
`MISSING_REQUIRED_DATA`、`decision=null`。

## 最新具完整 artifact／provenance 證據的橫截面研究推論


<!-- release-manifest:model-snapshot:start -->
...
模型 bundle 由最後一個 walk-forward fold 依固定規則建立，沒有用最新橫截面選模；相同 artifact、設定與 seed 的 bundle identity 可重現。這批資料是目前具完整 artifact／provenance 證據的回溯研究推論，不是新的 OOS 評估。Production API 的既有驗證紀錄顯示回傳 1,068 檔且每檔恰好 8 層 fail-closed 研究 gate；缺少可交易性、市場曝險或部位輸入時不得推測通過。舊發布欄位曾把 1,068 列記為 `NO_TRADE`；狀態稽核已將其權威語意重分類為 `MISSING_REQUIRED_DATA`，政策動作為空值。不得描述為正式候選股、即時可交易建議或獲利保證。
<!-- release-manifest:model-snapshot:end -->
