...
- 新增手機版 `/admin` 後台，整合同步進度、資料來源、修復佇列、缺漏分類、FinMind 近 60 分鐘配額、排行榜累積週期與最近事件。
- 可依上市／上櫃／ETF、待修復／錯誤及股票代號篩選，並一鍵複製純文字修復報告，直接提供給維護者定位問題。
- 權限由 Supabase Auth 使用者 UUID 與 `app_admins` RLS 名單雙重確認；隱藏前端入口不是授權依據，一般帳戶直接開啟網址仍無法讀取日誌。
- 預設管理員帳號名稱為 `Migao`。密碼只存在 Supabase Auth，不寫入 GitHub、HTML 或 JavaScript。Supabase 密碼至少需要 6 個字元，因此五字元的 `Migao` 不能作為正式密碼。
...
- 資料健康中心會顯示各來源日期、上市／上櫃／ETF 涵蓋率與修復佇列，並把缺漏分成「可重試、上游異常、官方期別落後、來源沒有、不適用」，不再只顯示空白。
...
先部署 `20260715174500_admin_operations_console.sql`，再於本機以 service-role key 執行一次建立腳本。帳號預設為 `Migao`；請自行指定至少 6 個字元的密碼：
...
腳本會建立或更新 `migao@admin.twss.local` 的 Supabase Auth 帳戶，並把其 UUID 寫入 `app_admins`。service-role key 與密碼都不可提交到 GitHub；執行後即可用 `Migao` 與剛設定的密碼登入 `/admin`。
...
- `GET /api/market-data?type=data-health`
