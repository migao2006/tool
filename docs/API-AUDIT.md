...
上述公開市場資料表皆啟用 RLS，訪客與一般登入者沒有新增、修改或刪除權限。`data_sync_status` 是未使用的舊表名，fresh migration 不再依賴它；正式同步進度只由管理端讀取 `stock_sync_state`，網站與公開快照均不輸出。排程經 Vault 權杖呼叫 Edge Function，函式再以伺服器角色寫入，並將健康／缺漏摘要寫入 `[admin-data-health]` 管理日誌。原始碼只包含 publishable key，不包含 service role key 或同步權杖。
