...
def main(argv: Sequence[str] | None = None) -> int:
    return run_feature_build(
        ...
        VenueFeatureBuildDependencies(
            ...
            archive_scope_filters=TPEX_ARCHIVE_SCOPE_FILTERS,
            failure_reason_code="TPEX_ARCHIVE_FEATURE_BUILD_FAILED",
            supabase_writer_factory=SupabaseWriter,
            manifest_repository_factory=HistoricalArchiveManifestRepository,
            identity_repository_factory=TpexCurrentIdentityRepository,
            r2_client_factory=R2Client.from_env,
            historical_reader_factory=HistoricalParquetReader,
            dataset_snapshot_hash=dataset_snapshot_hash,
            parquet_writer_factory=TpexArchiveFeatureParquetWriter,
            dataset_builder_factory=TpexArchiveFeatureDatasetBuilder,
            artifact_reader_factory=TpexFeatureArtifactReader,
        ),
    )
