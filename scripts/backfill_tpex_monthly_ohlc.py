"""Archive bounded official TPEx monthly OHLC tasks to private R2."""
...
from src.data.ingestion.tpex_ohlc_archive_service import (  # noqa: E402
    TpexOhlcArchiveService,
)
from src.data.ingestion.tpex_ohlc_backfill_coordinator import (  # noqa: E402
    TpexOhlcBackfillCoordinator,
)
from src.data.ingestion.tpex_ohlc_backfill_repository import (  # noqa: E402
    TpexOhlcBackfillRepository,
)
from src.data.ingestion.tpex_ohlc_landing_service import (  # noqa: E402
    TpexOhlcLandingService,
)
...
from src.data.providers.tpex import TpexClient  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Archive completed official TPEx monthly OHLC responses."
    )
    _ = parser.add_argument(
        "--start-month",
        type=_month,
        default=date(2012, 4, 1),
    )
...
def _worker_id() -> str:
    ...
    return f"tpex-ohlc-{run_id}-{attempt}-{uuid4().hex[:12]}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        summary = TpexOhlcBackfillCoordinator(
            repository=TpexOhlcBackfillRepository(writer),
            landing_service=TpexOhlcLandingService(
                provider=TpexClient(
                    http=JsonHttpClient(
                ...
                archive_service=TpexOhlcArchiveService(
                    store=store,
        ...
        result: dict[str, object] = {
            ...
            "reason_code": getattr(
                error,
                "reason_code",
                "TPEX_OHLC_BACKFILL_CONFIGURATION_ERROR",
            ),
            "message": str(error),
        }
        _write(output, result)
