...
const [edge, migration, manualMigration, unlimitedMigration, vaultMigration, opportunityEngine, deepData] = await Promise.all([
...
assert.match(edge, /GEMINI_TIMEOUT_MS = 90_000/);
assert.match(edge, /retryOptions: \{ attempts: 1 \}/, "provider calls must not auto-retry and amplify usage");
assert.doesNotMatch(edge, /claim\.action === "user_limit"|claim\.action === "global_limit"/);
...
assert.match(unlimitedMigration, /twss-ai-manual:/, "same-input requests must remain serialized");
assert.match(unlimitedMigration, /twss-ai-manual-concurrency/, "different symbols must share one atomic concurrency gate");
assert.match(unlimitedMigration, /v_active_count >= 2/);
assert.doesNotMatch(unlimitedMigration, /v_recent_count|twss_reserve_ai_calls|user_limit|global_limit/);
assert.match(unlimitedMigration, /'quotaMode', 'unlimited'/);
assert.match(unlimitedMigration, /interval '90 days'/);
assert.match(unlimitedMigration, /interval '30 days'/);
assert.doesNotMatch(unlimitedMigration, /grant execute[\s\S]*to anon|grant execute[\s\S]*to authenticated/);
...
console.log("AI research tests passed: unlimited manual flow, independent scoring, atomic deduplication/concurrency, retention, and no-key isolation");
