export const MAINTENANCE_RETRY_AFTER_SECONDS = 300;

export function maintenancePayload(state = {}) {
  return {
    ok: false,
    code: "MAINTENANCE",
    message: "系統正在安全升級，完成驗證後將恢復服務。",
    phase: state.phase || "maintenance",
    generation: state.generation !== null
      && state.generation !== undefined
      && Number.isFinite(Number(state.generation))
      ? Number(state.generation)
      : null,
    updatedAt: state.updated_at || state.updatedAt || null,
  };
}

export function maintenanceDocument(state = {}) {
  const payload = maintenancePayload(state);
  const statusText = payload.phase === "draining"
    ? "背景工作正在安全停止"
    : payload.phase === "verifying"
      ? "正在進行恢復前驗證"
      : "資料與模型正在升級";

  return `<!doctype html>
<html lang="zh-Hant">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <meta name="theme-color" content="#07131d">
  <meta name="robots" content="noindex,nofollow">
  <title>台股智選｜系統升級中</title>
  <style>
    :root{color-scheme:dark;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}
    body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at top,#0d2933,#06111a 58%);color:#eef8fb}
    main{width:min(520px,100%);padding:32px 28px;border:1px solid #234451;border-radius:24px;background:rgba(8,28,38,.94);box-shadow:0 24px 70px rgba(0,0,0,.35)}
    .eyebrow{margin:0 0 10px;color:#55ddc1;font-size:13px;font-weight:800;letter-spacing:.16em}h1{margin:0 0 14px;font-size:clamp(28px,7vw,40px);line-height:1.2}
    p{margin:0;color:#aec1c9;font-size:17px;line-height:1.75}.status{display:flex;align-items:center;gap:10px;margin-top:24px;padding-top:20px;border-top:1px solid #1c3945;color:#dcebee;font-size:14px}
    .dot{width:10px;height:10px;border-radius:50%;background:#f5bf45;box-shadow:0 0 0 6px rgba(245,191,69,.12)}
  </style>
</head>
<body><main><p class="eyebrow">SYSTEM MAINTENANCE</p><h1>系統安全升級中</h1><p>我們正在更新可驗證的機會排序、資料快照與風險控制。完成完整檢查後會立即恢復服務。</p><div class="status"><span class="dot" aria-hidden="true"></span>${statusText}</div></main></body>
</html>`;
}
