...
ACTIVE
## Authorization
`FULL_AUTONOMY_UNTIL_MAIN_UPDATE`, explicitly granted by the user. Stop before
updating `main` or another protected branch.
## Primary Outcome
...
summary.
## Background
The latest manual full update reached final verification but failed to download
`daily-research-production-{TWSE,TPEX}-<run>-1`. Evidence points to the artifact
producer/consumer boundary rather than market-data logic.
## Subtasks
...
- Commit, push the feature branch, create or update a Pull Request, and verify CI.
## Allowed Scope
...
must not alter model, data, publication, or validation semantics.
## Validation Plan
...
completion, or if an unrelated subsystem must change. Report truthfully after
three materially different failed attempts at one root cause.
## Definition of Done
...
- Added reusable outputs for `should_run`, `markets`, and `production_requested`.
  ...
  this invocation and still attempts/fails closed when a required artifact is absent.
- Added artifact lifecycle regression tests and updated the direct operator document.
...
- Pinned quality/security gate passed, including Ruff, basedpyright
  ...
  audit.
- Fast verification passed. Full verification ultimately passed with 1,115 Python
  and 66 Playwright tests. Two unrelated one-off browser failures each passed their
  exact rerun before the final clean Full run.
- Independent read-only review reported zero findings and made no changes.
- Commit, push, Pull Request, CI, terminal task archive, and final continuity update
  remain pending.
