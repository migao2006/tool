...
from src.data.ingestion.tpex_ohlc_backfill_contracts import (
...
def _task(
    month: int,
    *,
    dataset: str = "tpex_price_index_ohlc",
    market: str = "TPEX",
    symbol: str = "TPEX_INDEX",
...
def _snapshot(*, remaining: int = 0, exhausted: int = 0) -> TpexOhlcQueueSnapshot:
    return TpexOhlcQueueSnapshot(
...
class FakeRepository:
    def __init__(
        ...
        *,
        snapshot: TpexOhlcQueueSnapshot | None = None,
    ) -> None:
    ...
    def land(
        ...
        *,
        ...
        return TpexOhlcLandingResult(
            ...
            object_key=f"tpex/index/{backfill_task_id}.parquet",
...
def _coordinator(
    ...
    return TpexOhlcBackfillCoordinator(
        repository=repository,
        landing_service=landing,
        now_fn=lambda: datetime(2026, 7, 20, tzinfo=timezone.utc),
...
@pytest.mark.parametrize(
    "task",
    [
        _task(1, dataset="taiex_price_index_ohlc"),
        _task(1, market="TWSE", symbol="TAIEX"),
    ],
)
def test_worker_rejects_non_tpex_or_taiex_queue_task(
    ...
    assert captured.value.reason_code == "TPEX_OHLC_BACKFILL_TASK_SCOPE_INVALID"
...
def test_ingestion_failure_releases_task_to_retry_and_propagates() -> None:
    ...
    failure = IngestionError("TPEX_OHLC_PROVIDER_CONTRACT_FAILED", "bad payload")
    ...
    assert (
        repository.complete_calls[0]["error_code"]
        == "TPEX_OHLC_PROVIDER_CONTRACT_FAILED"
    )
