"""Coordinate a bounded, paced run of monthly official TPEx OHLC tasks."""
...
def _valid_task(
    ...
    *,
    ...
    return (
        task.source_dataset == TPEX_MONTHLY_OHLC_DATASET
        and task.symbol == TPEX_OHLC_SYMBOL
...
class TpexOhlcBackfillCoordinator:
    ...
    def run(
        ...
        *,
        ...
        self.repository.ensure_tpex_source()
        ...
        for task_index in range(max_tasks):
            ...
            if not _valid_task(
                ...
                self.repository.complete(
                    ...
                    error_code="TPEX_OHLC_BACKFILL_TASK_SCOPE_INVALID",
                )
                raise IngestionError(
                    "TPEX_OHLC_BACKFILL_TASK_SCOPE_INVALID",
                    "claimed task is outside the official monthly TPEx scope",
                )
            if task_index:
            ...
            try:
                ...
                self.repository.complete(
                    ...
                    success=False,
                    retry_after_seconds=retry_after_seconds,
                    error_code=getattr(error, "reason_code", "TPEX_OHLC_FETCH_FAILED"),
                )
                raise
            self.repository.complete(
                task=task,
        ...
        if queue.exhausted:
            ...
            outcome = "DEFERRED"
        return TpexOhlcBackfillSummary(
            outcome=outcome,
