"""Stream verified TWSE archives into a feature-only research dataset."""
...
from src.features.twse_price_volume_builder import build_twse_price_volume_features
...
from .twse_archive_feature_parquet import TwseArchiveFeatureParquetWriter
from .twse_archive_feature_rows import (
    SourceProvenance,
    archive_id,
    canonical_record,
    group_manifests,
...
@final
class TwseArchiveFeatureDatasetBuilder:
        ...
        *,
        ...
        try:
            ...
            if manifests.object_count == 0:
                raise TwseArchiveFeatureBuildError(
                    "TWSE_ARCHIVE_MANIFESTS_EMPTY",
                    "No scoped TWSE daily-bar archive manifests were returned",
                )
            if not identities.by_symbol:
                raise TwseArchiveFeatureBuildError(
                    "TWSE_CURRENT_IDENTITIES_EMPTY",
                    "No current TWSE common-stock identities were returned",
                )
            grouped = group_manifests(manifests.rows)
        ...
        try:
            for symbol, symbol_manifests in grouped.items():
                ...
                for raw_manifest in symbol_manifests:
                    ...
                    for row in archive.rows:
                        ...
                        records.append(canonical_record(row, identity=identity))
            ...
            if output_row_count == 0:
                raise TwseArchiveFeatureBuildError(
                    "TWSE_RESEARCH_FEATURE_ROWS_EMPTY",
                    "All archive rows failed the research feature gates",
                )
            audit = TwseArchiveFeatureAudit(
