-- v17.3 administrator-only operations console.
-- Passwords are owned by Supabase Auth and are never stored in this migration.
...
create table if not exists public.app_admins (
  ...
  constraint app_admins_username_format check (username ~ '^[A-Za-z0-9_.-]{3,32}$')
);

create unique index if not exists app_admins_username_lower_uidx
...
revoke all on function public.twss_is_admin() from public, anon;
grant execute on function public.twss_is_admin() to authenticated, service_role;

-- Quota reservations and in-progress ranking cycles are not exposed to normal
-- signed-in users.  These policies only widen read access for app_admins.
drop policy if exists twss_api_quota_admin_read on public.twss_api_quota_reservations;
create policy twss_api_quota_admin_read on public.twss_api_quota_reservations
  for select to authenticated
  using ((select public.twss_is_admin()));
grant select on public.twss_api_quota_reservations to authenticated;

drop policy if exists opportunity_ranking_cycles_admin_read on public.opportunity_ranking_cycles;
create policy opportunity_ranking_cycles_admin_read on public.opportunity_ranking_cycles
  for select to authenticated
  using ((select public.twss_is_admin()));
        ...
        pg_catalog.jsonb_strip_nulls(pg_catalog.jsonb_build_object(
          ...
          'lastErrorPreview', case when s.last_error is null then null else
            pg_catalog.regexp_replace(
              ...
              '(apikey|authorization|bearer|token)[^,;[:space:]]*',
              '[redacted]',
              'gi'
            )
          end,
...
revoke all on function public.twss_admin_operations_log(integer) from public, anon;
grant execute on function public.twss_admin_operations_log(integer) to authenticated, service_role;

comment on function public.twss_admin_operations_log(integer) is
  'Administrator-only sanitized sync, repair, quota and ranking-cycle log.';
