"""Supabase adapter for the isolated official TPEx monthly OHLC queue."""
...
from .tpex_ohlc_backfill_contracts import TpexOhlcQueueSnapshot


class TpexOhlcQueueWriter(Protocol):
    def upsert(
        self,
        table: str,
...
@final
class TpexOhlcBackfillRepository:
    def __init__(self, writer: TpexOhlcQueueWriter) -> None:
        self.writer = writer

    def ensure_tpex_source(self) -> None:
        source = next(
            row for row in data_source_rows() if row["source_code"] == "TPEX"
        )
        ...
        *,
        ...
        if isinstance(value, bool) or not isinstance(value, int):
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_RPC_INVALID",
                "TPEx OHLC task seeding returned an invalid result",
            )
        return value
    ...
    def claim(
        ...
        *,
        ...
        if not isinstance(raw, list):
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_RPC_INVALID",
                "TPEx OHLC task claim returned an invalid result",
            )
        rows = cast(list[object], raw)
        if len(rows) > 1 or any(not isinstance(row, Mapping) for row in rows):
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_RPC_INVALID",
                "TPEx OHLC task claim returned an invalid row collection",
            )
        return (
    ...
    def complete(
        ...
        *,
        ...
        if value is not True:
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_LEASE_LOST",
                "TPEx OHLC task completion was rejected",
            )
    ...
    def snapshot(
        ...
        *,
        ...
        if not isinstance(raw, list):
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_RPC_INVALID",
                "TPEx OHLC snapshot returned an invalid result",
            )
        rows = cast(list[object], raw)
        if len(rows) != 1 or not isinstance(rows[0], Mapping):
            raise IngestionError(
                "TPEX_OHLC_BACKFILL_RPC_INVALID",
                "TPEx OHLC snapshot did not return exactly one row",
            )
        return TpexOhlcQueueSnapshot.from_row(
