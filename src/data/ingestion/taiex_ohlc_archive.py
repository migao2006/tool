"""Build a manifest-ready TAIEX OHLC archive without performing external writes."""
...
from .taiex_ohlc_contracts import TAIEX_OHLC_SYMBOL, NormalizedTaiexOhlcBatch
from .taiex_ohlc_parquet import serialize_taiex_ohlc_parquet


def build_taiex_ohlc_archive(
    ...
    request = HistoricalArchiveRequest(
        ...
        source_symbol=TAIEX_OHLC_SYMBOL,
        requested_start_date=batch.requested_month,
        requested_end_date=end_date,
        source_payload_sha256=batch.source_payload_sha256,
        retrieved_at=batch.retrieved_at,
    )
    return serialize_taiex_ohlc_parquet(batch, request=request)


def build_taiex_ohlc_manifest(
    batch: NormalizedTaiexOhlcBatch,
    ...
    *,
    ...
    """Bind a verified object location to its compact research-only manifest."""
    ...
    if (
        ...
        raise ValueError("TAIEX artifact does not match the normalized batch")
