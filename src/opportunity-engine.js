...
function companyCategories(stock, deep, context, group) {
  ...
  return [
    category("growth", "營收與獲利成長", 30, [
      ...
      item("cash_conversion", "盈餘現金轉換", 7, financial.cashConversion, interpolate(financial.cashConversion, [[-0.5, 0], [0, 20], [0.5, 55], [1, 86], [1.5, 100]]), "現金流量表"),
...
function riskAssessment(stock, deep, currentRisk, group) {
  ...
  if (group !== "etf") {
    ...
    if (finite(financial.epsYoy) && financial.epsYoy > 0 && finite(financial.operatingCashFlow) && financial.operatingCashFlow < 0) {
      deduction += 6;
      flags.push("獲利成長但營業現金流為負");
    }
