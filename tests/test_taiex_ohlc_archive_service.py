...
from src.data.ingestion.taiex_ohlc_archive_service import TaiexOhlcArchiveService
...
def _service(
    ...
    return TaiexOhlcArchiveService(
...
def test_archive_is_immutable_read_back_verified_and_manifest_only() -> None:
    ...
    result = _service(store, writer).archive(batch, backfill_task_id=81)
    ...
    assert manifest["source_dataset"] == "taiex_price_index_ohlc"
    assert manifest["schema_version"] == "twse_taiex_price_index_ohlc.v1"
    assert manifest["source_symbol"] == "TAIEX"
    assert manifest["backfill_task_id"] == 81
...
def test_same_payload_retry_reuses_object_and_original_observation_time() -> None:
    ...
    first_batch = normalize_taiex_monthly_ohlc(_payload())
    first = _service(store, first_writer).archive(first_batch, backfill_task_id=81)
    original_observed_at = first_writer.rows[0]["first_observed_at"]
    retry_writer = RecordingWriter()
    retry_batch = normalize_taiex_monthly_ohlc(
        _payload(retrieved_at=datetime(2026, 7, 20, tzinfo=timezone.utc))
    )
...
def test_tampered_existing_object_fails_before_manifest_write() -> None:
    ...
    first_writer = RecordingWriter()
    batch = normalize_taiex_monthly_ohlc(_payload())
    result = _service(store, first_writer).archive(batch, backfill_task_id=81)
    stored = store.objects[result.object_key]
    stored.body = bytes([stored.body[0] ^ 1]) + stored.body[1:]
    retry_writer = RecordingWriter()

    with pytest.raises(IngestionError) as captured:
        _ = _service(store, retry_writer).archive(batch, backfill_task_id=81)
