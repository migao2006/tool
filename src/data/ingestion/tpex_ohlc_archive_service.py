"""Write and read-back verify one immutable monthly TPEx OHLC object."""
...
from .tpex_ohlc_archive import build_tpex_ohlc_archive, build_tpex_ohlc_manifest
...
def _observed_at(value: str | None) -> datetime:
    try:
        ...
        raise IngestionError(
            "TPEX_OHLC_R2_METADATA_INVALID",
            "TPEx OHLC object is missing a valid retrieval timestamp",
        ) from error
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise IngestionError(
            "TPEX_OHLC_R2_METADATA_INVALID",
            "TPEx OHLC object retrieval timestamp is timezone-naive",
        )
    ...
    def archive(
        ...
        *,
        ...
        if artifact.byte_size > self.max_object_bytes:
            raise IngestionError(
                "TPEX_OHLC_ARCHIVE_OBJECT_TOO_LARGE",
                "TPEx OHLC Parquet exceeds the configured object limit",
            )
        try:
            ...
            raise IngestionError(
                "TPEX_OHLC_R2_WRITE_FAILED",
                "TPEx OHLC object could not be written and inspected",
            ) from error
        if head is None:
            raise IngestionError(
                "TPEX_OHLC_R2_OBJECT_MISSING",
                "TPEx OHLC object is missing after conditional upload",
            )
        ...
        if not created:
            ...
            if not re.fullmatch(r"[0-9a-f]{64}", content_sha256) or row_count != str(
                len(batch.rows)
            ):
                raise IngestionError(
                    "TPEX_OHLC_R2_METADATA_INVALID",
                    "Existing TPEx OHLC object does not match the monthly batch",
                )
            manifest = replace(
                manifest,
                parquet_sha256=content_sha256,
                byte_size=head.content_length,
                first_observed_at=_observed_at(head.metadata.get("retrieved-at")),
            )
        try:
        ...
        if verified.row_count != len(batch.rows):
            raise IngestionError(
                "TPEX_OHLC_ARCHIVE_ROW_COUNT_MISMATCH",
                "Verified TPEx OHLC rows do not match the provider response",
            )
        self.repository.save(_manifest_row(manifest, backfill_task_id=backfill_task_id))
