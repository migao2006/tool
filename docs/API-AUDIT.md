...
| Dataset | 範圍 | 衍生用途 |
| --- | ---: | --- |
| `TaiwanStockPrice` | 18 個月，最多保留 280 個交易日 | 均線、斜率、突破、量能、ATR、RSI、MACD、KD、相對強弱、還原異常 |
...
| `TaiwanStockFinancialStatements` | 52 個月 | 8～12 季 EPS、營收、利潤率與業外品質 |
| `TaiwanStockBalanceSheet` | 52 個月 | ROE、負債、流動比率、存貨與應收變化 |
| `TaiwanStockCashFlowsStatement` | 52 個月 | 營業現金流、自由現金流與現金轉換 |
...
每日排程先用全市場快照縮小成上市 10、上櫃 10、ETF 10 檔上限，再逐檔深度驗證。全新候選池第一次最多約 172 次 FinMind dataset 請求（20 檔公司股各 8 個 dataset、10 檔 ETF 價量、兩個長期市場基準）；後續若月營收與財報期別未改變，直接重用前次分析，只更新價量、法人、融資融券與借券，通常約 92 次。候選變動或新期別出現時才補抓對應歷史。
...
- 不足 25 個不同交易日不公布回測成果。
- `scripts/audit-live.mjs` 會核對主要端點日期與必要欄位；`npm test` 會驗證建置、引擎硬規則、缺漏重正規化、ETF 分榜與資料源失效備援。
