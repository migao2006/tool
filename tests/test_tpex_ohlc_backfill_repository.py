...
from src.data.ingestion.tpex_ohlc_backfill_repository import (
    TpexOhlcBackfillRepository,
)
...
def _task_row() -> dict[str, object]:
    return {
        ...
        "display_name": "TPEx Price Index OHLC",
        "market": "TPEX",
...
def test_repository_uses_only_the_isolated_tpex_rpcs() -> None:
    writer = FakeWriter()
    writer.responses.update(
        {
            "seed_tpex_price_index_ohlc_tasks": 3,
            "claim_tpex_price_index_ohlc_task": [_task_row()],
            "complete_tpex_price_index_ohlc_task": True,
            "tpex_price_index_ohlc_backfill_snapshot": [_snapshot_row()],
        }
    )
    repository = TpexOhlcBackfillRepository(writer)
    snapshot_at = datetime(2026, 7, 20, 1, tzinfo=timezone.utc)
    token = UUID("11111111-1111-4111-8111-111111111111")

    repository.ensure_tpex_source()
    ...
    assert source_rows[0]["source_code"] == "TPEX"
    ...
    assert all("taiex" not in name for name, _ in writer.rpcs)
    ...
    assert writer.rpcs[2][1]["p_task_id"] == 29
...
@pytest.mark.parametrize(
    ...
    [
        ("seed_tpex_price_index_ohlc_tasks", True, "seed"),
        ("claim_tpex_price_index_ohlc_task", {}, "claim"),
        ("complete_tpex_price_index_ohlc_task", False, "complete"),
        ("tpex_price_index_ohlc_backfill_snapshot", [], "snapshot"),
    ],
)
