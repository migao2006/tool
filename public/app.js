...
const S={
  ...
  historyCache:new Map(),aiResearchCache:new Map(),aiResearchState:new Map(),historySignals:new Map(),backtestCache:new Map(),detailSymbol:null,forecastQuery:'',verifyQuery:'',verifySymbol:'',
  mineSub:'watch',session:null,dataStatus:{},sourceDates:{},fundDates:{},syncState:'本機模式',pendingAiSymbol:null
};
...
async function getAiResearch(symbol,refresh=false){
  const cached=S.aiResearchCache.get(symbol);if(!refresh&&cached)return cached instanceof Promise?cached:Promise.resolve(cached);
  const promise=(async()=>{const payload=await fetchJson(`${EDGE}?type=ai-research&symbol=${encodeURIComponent(symbol)}${refresh?'&refresh=1':''}`,30000,1);const result=payload?.available?payload:{available:false,symbol,reason:payload?.reason||'not-generated'};S.aiResearchCache.set(symbol,result);return result})();
  S.aiResearchCache.set(symbol,promise);try{return await promise}catch(error){S.aiResearchCache.delete(symbol);throw error}
}

async function postAiResearch(symbol){
  if(!await refreshSession()){const error=new Error('請先登入再產生 AI 研究摘要');error.code='LOGIN_REQUIRED';throw error}
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),150000);
  try{
    const response=await fetch('/api/ai-research',{method:'POST',cache:'no-store',signal:controller.signal,headers:{accept:'application/json','content-type':'application/json',authorization:`Bearer ${S.session.access_token}`},body:JSON.stringify({symbol})});
    let payload=null;try{payload=await response.json()}catch{}
    if(!response.ok){const error=new Error(payload?.error||`HTTP ${response.status}`);error.status=response.status;error.code=payload?.code||null;error.retryAfterAt=payload?.retryAfterAt||null;throw error}
    return payload
  }catch(error){if(error.name==='AbortError'){const timeoutError=new Error('AI 研究處理逾時，請稍後再試');timeoutError.code='AI_TIMEOUT';throw timeoutError}throw error}
  finally{clearTimeout(timer)}
}

async function requestAiResearch(symbol,force=false){
  if(S.aiResearchState.get(symbol)?.aiLoading)return;
  S.aiResearchState.set(symbol,{aiLoading:true});if(S.detailSymbol===symbol)openDetail(symbol,false);
  try{
    let result=force?null:await getAiResearch(symbol,true);
    if(!result?.available)result=await postAiResearch(symbol);
    if(!result?.available){S.aiResearchCache.delete(symbol);result=await getAiResearch(symbol,true)}
    if(!result?.available)throw new Error('摘要已送出但尚未完成，請稍後再試');
    S.aiResearchCache.set(symbol,result);S.aiResearchState.set(symbol,{aiResearch:result,aiLoading:false})
  }catch(error){
    S.aiResearchState.set(symbol,{aiLoading:false,aiNeedsLogin:error.code==='LOGIN_REQUIRED',aiError:error.message,aiErrorCode:error.code||null,retryAfterAt:error.retryAfterAt||null})
  }
  if(S.detailSymbol===symbol)openDetail(symbol,false)
}
...
function aiResearchHtml(state,stock){
  ...
  const scenarioRows=[['偏多情境',scenarios.bullish,'good'],['中性情境',scenarios.neutral,'base'],['風險情境',scenarios.bearish,'bad']];
  const verdictClass=analysis.verdict==='偏多觀察'?'up':analysis.verdict==='風險升高'?'down':'neutral';
  const generated=payload.generatedAt?new Date(payload.generatedAt).toLocaleString('zh-TW',{timeZone:'Asia/Taipei',hour12:false}):'—';
  return`<h3 class="section-title">Gemini AI 研究摘要</h3><div class="card ai-card accent"><div class="ai-panel-head"><div><span class="tag info">獨立分析 · 不影響機會分數</span><div class="ai-verdict ${verdictClass}">${esc(analysis.verdict||payload.verdict||'中性觀察')}</div></div><div class="ai-confidence"><small class="muted">AI 摘要信心</small><b>${fmt(analysis.aiConfidence??payload.aiConfidence,0)}%</b></div></div><p class="ai-summary">${esc(analysis.summary||'')}</p><div class="ai-columns"><section><h4>支持觀察</h4>${aiEvidenceHtml(analysis.positives,'未列出額外正向因素')}</section><section><h4>主要風險</h4>${aiEvidenceHtml(analysis.risks,'未列出額外風險')}</section></div><div class="ai-scenarios">${scenarioRows.map(([label,item,type])=>`<div class="ai-scenario ${type}"><b>${label}</b><span>條件：${esc(item?.condition||'資料不足')}</span><small>${esc(item?.observation||'持續觀察公開資料變化')}</small></div>`).join('')}</div>${Array.isArray(analysis.watchItems)&&analysis.watchItems.length?`<h4>後續觀察</h4><div class="rules">${analysis.watchItems.map(item=>`<span>${esc(item)}</span>`).join('')}</div>`:''}${Array.isArray(analysis.dataWarnings)&&analysis.dataWarnings.length?`<div class="ai-warnings"><b>資料提醒</b>${analysis.dataWarnings.map(item=>`<span>${esc(item)}</span>`).join('')}</div>`:''}<div class="ai-meta">資料日 ${esc(payload.dataDate||'—')} · 產生時間 ${esc(generated)} · ${esc(payload.model||'Gemini')}</div><button class="btn secondary ai-action" data-ai-research="${esc(stock.symbol)}" data-ai-refresh="1">更新 AI 摘要</button><div class="notice ai-disclaimer">AI 僅整理既有公開資料與可能情境，不會修改系統分數、排行榜或原本分析，也不構成投資建議。</div></div>`
}
...
async function openDetail(symbol,loadHistory=true){
  ...
  const cachedAi=S.aiResearchCache.get(symbol),resolvedAi=cachedAi&&!(cachedAi instanceof Promise)?cachedAi:null;
  ...
  const initialAiState=resolvedAi?.available?{aiResearch:resolvedAi,aiLoading:false}:{aiLoading:false};
  const paint=()=>{if(S.detailSymbol!==symbol)return;const scrollTop=q('.sheet',modalRoot)?.scrollTop||0,aiState=S.aiResearchState.get(symbol)||initialAiState;modalRoot.innerHTML=detailHtml(stock,{...historyState,...aiState});bindModal();const sheet=q('.sheet',modalRoot);if(sheet)sheet.scrollTop=scrollTop};
  paint();
  const tasks=[];
...
function closeModal(){
  S.detailSymbol=null;S.pendingAiSymbol=null;modalRoot.innerHTML='';document.body.classList.remove('modal-open');
  const returnFocus=modalReturnFocus;modalReturnFocus=null;
...
function bindModal(){
  ...
  qa('[data-ai-research]',modalRoot).forEach(button=>button.onclick=e=>{e.stopPropagation();requestAiResearch(button.dataset.aiResearch,button.dataset.aiRefresh==='1')});
  qa('[data-ai-login]',modalRoot).forEach(button=>button.onclick=e=>{e.stopPropagation();S.pendingAiSymbol=button.dataset.aiLogin;openAccountModal()});
...
function openAccountModal(){
  ...
  modalRoot.innerHTML=`<div class="modal"><div class="sheet"><button class="sheet-close">×</button><h2>登入台股智選</h2><p class="muted">登入後可同步紀錄，並安全地手動產生 AI 研究摘要。</p><label>電子郵件<input id="authEmail" type="email" autocomplete="email"></label><label>密碼<input id="authPass" type="password" autocomplete="current-password" placeholder="至少 6 個字元"></label><div class="row" style="margin-top:12px"><button id="loginBtn" class="btn grow">登入</button><button id="signupBtn" class="btn secondary">建立帳戶</button></div><div id="authMsg" class="muted" style="margin-top:10px"></div></div></div>`;bindModal();
  const afterAuth=()=>{const pending=S.pendingAiSymbol;S.pendingAiSymbol=null;closeModal();render();if(pending){S.aiResearchState.delete(pending);openDetail(pending,false);requestAiResearch(pending)}};
