"""Archive one five-year FinMind TAIEX total-return response to private R2."""
...
from src.data.ingestion.historical_benchmark_coordinator import (  # noqa: E402
    HistoricalBenchmarkBackfillCoordinator,
)
from src.data.ingestion.historical_benchmark_landing_service import (  # noqa: E402
    HistoricalBenchmarkLandingService,
)
from src.data.ingestion.historical_benchmark_repository import (  # noqa: E402
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Archive one FinMind TAIEX total-return response to R2."
    )
...
def _worker_id() -> str:
    ...
    return f"benchmark-{run_id}-{attempt}-{uuid4().hex[:12]}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        archive = HistoricalDailyBarArchiveService(
        ...
        summary = HistoricalBenchmarkBackfillCoordinator(
            repository=HistoricalBenchmarkBackfillRepository(writer),
            landing_service=HistoricalBenchmarkLandingService(
                provider=provider,
        ...
        result: dict[str, object] = {
            ...
            "reason_code": getattr(
                error,
                "reason_code",
                "HISTORICAL_BENCHMARK_CONFIGURATION_ERROR",
            ),
            "message": str(error),
        }
        _write(output, result)
