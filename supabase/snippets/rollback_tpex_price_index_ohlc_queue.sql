...
begin
    if exists (
        ...
        raise exception using
            errcode = '55000',
            message = 'rollback blocked: TPEX OHLC queue or archive records exist';
    end if;
end
$guard$;

drop function if exists market_data.seed_tpex_price_index_ohlc_tasks(
    date,
    date,
    timestamptz
);
drop function if exists market_data.claim_tpex_price_index_ohlc_task(
    text,
    uuid,
    integer
);
drop function if exists market_data.complete_tpex_price_index_ohlc_task(
    bigint,
    uuid,
    boolean,
    date,
...
drop function if exists market_data.tpex_price_index_ohlc_backfill_snapshot(
    date,
    date
);
...
market_data.historical_backfill_tasks_tpex_month_claim_idx;
-- noqa: enable=PG01
