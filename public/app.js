...
const S={
  ...
  historyCache:new Map(),aiResearchCache:new Map(),historySignals:new Map(),backtestCache:new Map(),detailSymbol:null,forecastQuery:'',verifyQuery:'',verifySymbol:'',
...
async function getAiResearch(symbol){
  const cached=S.aiResearchCache.get(symbol);if(cached)return cached instanceof Promise?cached:Promise.resolve(cached);
  const promise=(async()=>{const payload=await fetchJson(`${EDGE}?type=ai-research&symbol=${encodeURIComponent(symbol)}`,30000,1);const result=payload?.available?payload:{available:false,symbol,reason:payload?.reason||'not-selected-or-not-generated'};S.aiResearchCache.set(symbol,result);return result})();
...
function aiResearchHtml(state){
  if(state?.aiLoading)return`<h3 class="section-title">Gemini AI 研究摘要</h3><div class="card ai-card"><div class="ai-panel-head"><b>獨立 AI 研究</b><span class="tag info">不影響機會分數</span></div><div class="loading"><span class="spinner"></span>正在讀取限量 AI 摘要…</div></div>`;
  if(state?.aiError)return`<h3 class="section-title">Gemini AI 研究摘要</h3><div class="card ai-card warn-card"><div class="ai-panel-head"><b>AI 摘要暫時無法讀取</b><span class="tag info">量化結果未受影響</span></div><p class="muted">${esc(state.aiError)}</p></div>`;
  const payload=state?.aiResearch;
  if(!payload?.available)return`<h3 class="section-title">Gemini AI 研究摘要</h3><div class="card ai-card"><div class="ai-panel-head"><b>目前未列入限量 AI 精選名單</b><span class="tag info">不影響機會分數</span></div><p class="muted">AI 每日只分析少量正式候選，未產生摘要是正常狀態；原本的量化排名與趨勢預測仍完整保留。</p></div>`;
...
async function openDetail(symbol,loadHistory=true){
  ...
  let aiState=resolvedAi?{aiResearch:resolvedAi,aiLoading:false}:{aiLoading:true};
  const paint=()=>{if(S.detailSymbol!==symbol)return;const scrollTop=q('.sheet',modalRoot)?.scrollTop||0;modalRoot.innerHTML=detailHtml(stock,{...historyState,...aiState});bindModal();const sheet=q('.sheet',modalRoot);if(sheet)sheet.scrollTop=scrollTop};
  paint();
  const tasks=[];
  ...
  const aiPromise=cachedAi instanceof Promise?cachedAi:(resolvedAi?null:getAiResearch(symbol));
  if(aiPromise)tasks.push(Promise.resolve(aiPromise).then(result=>{aiState={aiResearch:result,aiLoading:false};paint()}).catch(error=>{aiState={aiLoading:false,aiError:error.message};paint()}));
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js?v=16.4-ai1',{updateViaCache:'none'}).catch(()=>{});
