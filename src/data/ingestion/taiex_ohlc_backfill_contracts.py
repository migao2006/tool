"""Typed contracts for the official monthly TAIEX OHLC backfill."""
...
from .taiex_ohlc_contracts import TAIEX_OHLC_REASON_CODES
...
def _nonnegative_int(row: Mapping[str, object], name: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise IngestionError(
            "TAIEX_OHLC_BACKFILL_SNAPSHOT_INVALID",
            f"TAIEX OHLC queue snapshot contains invalid {name}",
        )
    return value


@dataclass(frozen=True)
class TaiexOhlcLandingResult:
    fetched_rows: int
    archived_rows: int
...
class TaiexOhlcQueueSnapshot:
    ...
    @classmethod
    def from_row(cls, row: Mapping[str, object]) -> "TaiexOhlcQueueSnapshot":
        names = (
            "task_count",
            "pending",
            "leased",
...
@dataclass(frozen=True)
class TaiexOhlcBackfillSummary:
    outcome: str
    start_month: str
    end_month: str
    attempted_tasks: int
