...
- 新增 Supabase Postgres 持久化後端。全市場 2,126 檔的每日快照會先入庫，再由游標分批深度驗證；排行榜不再被單次 GitHub 工作流程的「每組 10 檔」上限鎖住。
- 上市、上櫃、ETF 使用三個獨立游標與錯開排程，每批最多 2 檔。失敗只記錄該檔，下一批仍可繼續，並保留 `processed_count`、`cursor_offset`、最後成功時間與錯誤內容。
- 每個完成深度驗證的公司股保存最多 280 日價量、40 月營收、12 季財務、30 日法人及 30 日融資融券；前端先讀後端，資料不足時才回退到原有即時來源。
- 前端顯示後端已驗證檔數與同步狀態，並把後端累積榜和 GitHub 靜態快照合併；初期仍可用原本每組 10 檔的靜態資料，不會因後端尚未跑完而空白。
- 資料表全面啟用 RLS：訪客只有讀取公開研究資料的權限，批次寫入必須由 Vault 保管的同步權杖與伺服器密鑰完成。

- 修正股票明細頁的市場路由：上櫃歷史行情不再誤送至 TWSE，改由 `market=上櫃` 的 FinMind 單一日線請求取得；每日快照另保留 180 日備援。
...
FinMind 請求使用單一佇列，預設每次啟動至少間隔 1.35 秒；4xx 不重送，429、逾時與 5xx 才有限次數退避重試。TWSE、TPEx、MOPS、TDCC 也各自排隊，不會無限制平行請求。月營收與財報期別未改變時會重用前一份分析，每日只刷新價量與籌碼；新期別或新候選才補抓歷史。後端每次只處理 2 檔，上市、上櫃與 ETF 分別在不同分鐘啟動，避免瞬間大量請求。

## 持久化後端

目前正式後端已啟用。平日台北時間 14:45 更新全市場快照，三組深度工作每 30 分鐘各跑一次、每次最多 2 檔，並從上次 `cursor_offset` 繼續。第一輪完整覆蓋需要時間；已完成的檔案會立刻進入累積排行榜，不必等整輪跑完。
...
部分 ETF 欄位沒有穩定且一致的免金鑰公開介面，包括即時淨值折溢價、追蹤誤差、完整內扣費用與成分集中度。v16.2 會顯示缺漏並降低 ETF 信心，不會拿公司基本面代替。
