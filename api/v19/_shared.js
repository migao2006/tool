import { V19PublicError } from "../../src/v19-backend.js";
...
export async function handleV19(request, reader) {
  ...
  if (request.method !== "GET") {
    return json({ error: { code: "method_not_allowed", message: "GET only" } }, 405, { allow: "GET, OPTIONS" });
  }
  try {
    return json(await reader(new URL(request.url)));
  } catch (error) {
    if (error instanceof V19PublicError) {
      return json({ error: { code: error.code, message: error.message } }, error.status);
    }
    console.error("[v19-api] request failed", {
...
export const v19ApiInternals = { json, CACHE_CONTROL, CDN_CACHE_CONTROL };
