    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      ...
      try {
        const headers = {
          ...
          "user-agent": "TaiwanStockSmartPicker/16.0",
        };
...
function financialSummary(incomeRows, balanceRows, cashRows) {
  ...
  const revenueValues = standalone(incomes, (row) => statementValue(row, ["Revenue"], /營業收入|收益合計/));
  const grossValues = standalone(incomes, (row) => statementValue(row, ["GrossProfit"], /營業毛利/));
  const operatingValues = standalone(incomes, (row) => statementValue(row, ["OperatingIncome"], /營業利益/));
  const netValues = standalone(incomes, (row) => statementValue(row, ["IncomeAfterTaxes", "ProfitLoss"], /本期淨利|本期稅後/));
  const epsValues = standalone(incomes, (row) => statementValue(row, ["EPS"], /每股盈餘/));
  const nonOperatingValues = standalone(incomes, (row) => statementValue(row, ["TotalNonoperatingIncomeAndExpense"], /營業外收入及支出/));
  const interestValues = standalone(incomes, (row) => statementValue(row, ["FinanceCosts", "InterestExpense"], /財務成本|利息費用/));
