...
- GitHub 推送不得被視為同時授權其他平台自動或手動部署；若 GitHub 整合會觸發外部部署，必須先向使用者說明並取得明確同意。
- 發布前必須確認 Git 差異、測試結果與提交範圍；只提交本次任務相關檔案，不得夾帶其他修改。
- 若 GitHub 推送受登入、憑證、權限或網路問題阻擋，應回報阻塞，不得改用其他部署平台繞過。
