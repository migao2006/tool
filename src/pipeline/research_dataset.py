...
class PreparedResearchDataset:
    ...
    def from_frame(
        ...
        *,
        ...
        normalized_features = tuple(dict.fromkeys(str(name) for name in feature_names))
        if not normalized_features or any(not name for name in normalized_features):
            raise ResearchDatasetError(
                "at least one named research feature is required"
            )
        required = {*REQUIRED_RESEARCH_COLUMNS, *normalized_features}
        ...
        if not availability_basis.isin(
            ("SOURCE_AVAILABLE_AT", "SCHEDULING_HINT")
        ).all():
        ...
        if (
            "data_quality_status" in prepared.columns
            and (
                prepared["data_quality_status"].astype(str).str.upper() == "HARD_FAIL"
            ).any()
        ):
            raise ResearchDatasetError(
                "HARD_FAIL rows must be excluded before training"
            )
        if (
            not prepared["data_quality_status"]
            .astype(str)
            .str.upper()
            .isin({"PASS", "WARN"})
            .all()
            ...
            raise ResearchDatasetError(
                "research rows require PASS or WARN data quality"
            )
        ...
        if not (prepared["usage_scope"].astype(str) == "MODEL_RESEARCH_ONLY").all():
            raise ResearchDatasetError(
                "prepared rows require MODEL_RESEARCH_ONLY scope"
            )
        ...
        for value in prepared["reason_codes"]:
            ...
            normalized_reasons.append(tuple(dict.fromkeys(value)))
        prepared["reason_codes"] = normalized_reasons
        ...
        for name in normalized_features:
            ...
            if not non_missing.map(lambda value: isfinite(float(value))).all():
                raise ResearchDatasetError(
                    f"{name} must contain finite values when present"
                )
        ...
        duplicate_key = prepared.duplicated(subset=["symbol", "decision_date"])
