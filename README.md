...
台股智選是「可驗證的台股機會排序系統」，不是 AI 猜漲跌。v20.1 以不可修改的 point-in-time 推薦快照為公開讀取來源，分開計算短波段（2／3／5／10 個交易日）與中期（前台 10／20／40 個交易日，約 2～8 週）候選，並以交易成本、下跌風險與換手懲罰調整為成本後機會排名。60 日只保留為研究訊號，不進入公開中期排行榜。
