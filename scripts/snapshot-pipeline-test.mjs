...
for (let index = 0; index < 45; index += 1) {
  ...
  const payload = {
    version: "17.1", dataDate: date, snapshotCoverage: { backtestReady: true },
...
assert.match(serviceWorker, /twss-v17\.1/);
assert.match(indexHtml, /\?v=17\.1/);
assert.match(appSource, /sw\.js\?v=17\.1/);
assert.match(smartSource, /backtest\.json\?v=17\.1/);
...
console.log("Snapshot pipeline tests passed: 25 mature-date gating, 45-day readiness, next-open runtime, final-cycle readiness, official open, and v17.1 cache busting");
