  ...
  function watchlistPageV20() {
    ...
    return `<div class="v20-watch-page">${pageHero('WATCHLIST', '自選股', '只追蹤你關心的股票、重要變化與風險，不記錄成本或損益。')}
      ${rows.length ? `<div class="v20-card-list">${rows.map(({ item, detail, short, medium, stock }) => {
        const reminder = first(v20.watchErrors.get(String(item.symbol)), safeArray(short?.risks)[0], safeArray(medium?.risks)[0], safeArray(short?.reasons)[0], safeArray(medium?.reasons)[0], detail ? '目前沒有已驗證的新提醒。' : '正在背景載入 AI 分析。');
