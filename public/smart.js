(() => {
  ...
  async function loadSnapshot(force = false) {
    ...
    const backtestPromise = fetch(`/api/market-data?type=ranking-backtest${force ? '&refresh=1' : ''}`, { cache: 'no-store' })
      ...
      .then(async payload => payload?.byGroup ? payload : fetch('/data/backtest.json?v=17.3.1', { cache: 'no-store' })
        .then(response => response.ok ? response.json() : null).catch(() => null))
      .catch(() => fetch('/data/backtest.json?v=17.3.1', { cache: 'no-store' })
        .then(response => response.ok ? response.json() : null).catch(() => null));
    try {
    ...
    if (!backendSnapshot) {
      try {
        const response = await fetch('/data/latest.json?v=17.3.1', { cache: 'no-store' });
