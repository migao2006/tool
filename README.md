...
v16.4 沒有修改 v16.3 的量化公式、深度資料、機會分數、排行榜或趨勢預測。Gemini 是一條完全獨立的後端支線，只讀正式候選的既有公開資料，將結構化摘要寫入 `ai_stock_research`，前端在股票明細顯示「Gemini AI 研究摘要」。

- 只挑選 `official=true`、分數至少 65、資料信心至少 70% 的標的；上市／上櫃／ETF 每日預設配額為 5／5／2。
- 每日預設最多 12 次、資料庫硬上限 20 次；兩個並行通道，不對每一檔股票分析。
- 輸入雜湊、模型與 schema 都沒變時直接跳過，不重複花費。
...
`ai-research` 查不到資料時會回傳 HTTP 200 與 `available:false`，代表該檔尚未被限量 AI 批次選中，不是網站錯誤。
