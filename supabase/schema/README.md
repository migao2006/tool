...
> 2026-07-19 核對：遠端 Production migration history 截止 `20260718191828`。本機 20260719 的 11 筆 forward migration 尚未套用；完整狀態見 [`../../docs/current-status.md`](../../docs/current-status.md)。
...
資料庫版本的 source of truth，也不是全部 migration 的完整清單：
...
`012`～`016` 以及 supplemental／benchmark archive 對應的 20260719 migrations 都尚未套用 Production；不只是上述兩個 schema。它們必須先在非正式環境完成 migration、代表性資料及 rollback 驗證。
...
目前遠端 migration history 沒有 `20260717180000` baseline；完成對齊前禁止直接執行
`supabase db push`、`--include-all` 或盲目 migration repair。
