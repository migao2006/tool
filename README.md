...
## v16.6：不限每日次數的手動 Gemini AI 研究摘要

v16.6 沒有修改 v16.3 的量化公式、深度資料、機會分數、排行榜或趨勢預測。Gemini 是一條完全獨立的後端支線；股票明細只顯示「AI 研究摘要」按鈕，使用者按下後才讀取快取或產生新摘要。

- 取消自動候選篩選與平日 AI 排程；上市、上櫃、ETF 只要後端深度資料已完成，都可手動要求摘要。
- 開啟股票明細不會呼叫 AI；按下按鈕才會先查快取，沒有可用摘要時才呼叫 Gemini。
- 產生新摘要需要登入，但沒有每人或全站每日次數上限；同時最多產生兩份，第三份會先回覆忙碌，避免併發重複與後端過載。
- 輸入雜湊、模型與 schema 都沒變時直接回傳 14 天快取，不重複花費。
- Google Gemini 帳戶本身的速率、用量與帳單限制仍然適用；後端不會自動重試放大請求，單次 Gemini 等待最多 90 秒。
...
完整架構、快取／併發保護與啟用方式請見 [docs/AI-RESEARCH.md](docs/AI-RESEARCH.md)。
...
- `ai_stock_research`、`ai_research_runs`、`ai_research_usage`：獨立 AI 摘要、執行紀錄與舊管理批次呼叫統計
