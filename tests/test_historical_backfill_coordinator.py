...
def test_secondary_worker_can_skip_shared_common_task_seeding() -> None:
    repository = FakeRepository([task(1, "2330", "TWSE", "COMMON_STOCK")])
    settings = HistoricalBackfillSettings(
        seed_common_tasks=False,
        seed_delisted_tasks=True,
    )
    ...
    assert repository.seed_delisted_common_calls == 0


def test_primary_worker_does_not_seed_delisted_tasks_before_feature_enablement() -> None:
    repository = FakeRepository([task(1, "2330", "TWSE", "COMMON_STOCK")])
    ...
    assert summary.succeeded_tasks == 1
    assert repository.seed_common_calls == 1
    assert repository.seed_delisted_common_calls == 0


def test_primary_worker_seeds_delisted_tasks_when_feature_is_enabled() -> None:
    repository = FakeRepository([task(1, "2330", "TWSE", "COMMON_STOCK")])
    settings = HistoricalBackfillSettings(seed_delisted_tasks=True)
    ...
    assert summary.succeeded_tasks == 1
    assert repository.seed_common_calls == 1
    assert repository.seed_delisted_common_calls == 1
