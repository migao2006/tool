...
def _feature_manifest(row_count: int) -> TwseFeatureArtifactManifest:
    return TwseFeatureArtifactManifest(
        parquet_sha256="1" * 64,
        parquet_schema_sha256="2" * 64,
        byte_size=1,
        row_count=row_count,
        dataset_version="twse-archive-price-volume-5d-v2",
        dataset_snapshot_sha256=DATASET_SNAPSHOT,
        source_archive_snapshot_sha256=SOURCE_SNAPSHOT,
        current_identity_snapshot_sha256=IDENTITY_SNAPSHOT,
        feature_schema_version=TWSE_PRICE_VOLUME_FEATURE_SCHEMA_VERSION,
...
def _in_memory_cross_section() -> LatestTwseFeatureCrossSection:
    import pandas as pd

    rows = [_row("2330"), _row("2317", close=150.0)]
    return LatestTwseFeatureCrossSection(
        frame=pd.DataFrame(rows),
        as_of_date=DEFAULT_ROW_DATE,
        manifest=_feature_manifest(len(rows)),
        market="TWSE",
    )
